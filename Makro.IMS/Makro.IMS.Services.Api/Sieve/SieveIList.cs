...
{
    public class SieveIListProcessor : SieveIListProcessor<SieveModel, FilterTerm, SortTerm>, ISieveIListProcessor
    {
        public SieveIListProcessor(IOptions<SieveOptions> options)
            : base(options)
        {
        }

        public SieveIListProcessor(IOptions<SieveOptions> options, ISieveCustomSortMethods customSortMethods)
            : base(options, customSortMethods)
        {
        }

        public SieveIListProcessor(IOptions<SieveOptions> options, ISieveCustomFilterMethods customFilterMethods)
    ...
    public class SieveIListProcessor<TFilterTerm, TSortTerm> :
        SieveIListProcessor<SieveModel<TFilterTerm, TSortTerm>, TFilterTerm, TSortTerm>, ISieveIListProcessor<TFilterTerm, TSortTerm>
    ...
    public class SieveIListProcessor<TSieveModel, TFilterTerm, TSortTerm> : ISieveIListProcessor<TSieveModel, TFilterTerm, TSortTerm>
        ...
        where TSortTerm : ISortTerm, new()
    {
        ...
        public List<TEntity> Apply<TEntity>(TSieveModel model, List<TEntity> source,
        ...
        protected virtual List<TEntity> ApplyFiltering<TEntity>(TSieveModel model, List<TEntity> result,
            object[] dataForCustomMethods = null)
        {
            ...
            return outerExpression == null
                ? result
                : result.AsQueryable<TEntity>().Where(Expression.Lambda<Func<TEntity, bool>>(outerExpression, parameter)).ToList();
        ...
        protected virtual List<TEntity> ApplySorting<TEntity>(TSieveModel model, List<TEntity> result,
            object[] dataForCustomMethods = null)
        {
            ...
            foreach (var sortTerm in model.GetSortsParsed())
            {
                ...
                if (property != null)
                {
                    result = result.AsQueryable().
                        OrderByDynamic(fullName, property, sortTerm.Descending, useThenBy, Options.Value.DisableNullableTypeExpressionForSorting).ToList();
                }
                else
                {
        ...
        protected virtual List<TEntity> ApplyPagination<TEntity>(TSieveModel model, List<TEntity> result)
        {
            ...
            result = result.Skip((page - 1) * pageSize).ToList();
            result = result.Take(Math.Min(pageSize, maxPageSize)).ToList();
        ...
        private List<TEntity> ApplyCustomMethod<TEntity>(List<TEntity> result, string name, object parent,
            object[] parameters, object[] optionalParameters = null)
        {
            ...
            if (customMethod == null)
            {
                ...
                if (genericCustomMethod != null &&
                    ...
                    genericCustomMethod.ReturnType.GetGenericTypeDefinition() == typeof(List<>))
                {
