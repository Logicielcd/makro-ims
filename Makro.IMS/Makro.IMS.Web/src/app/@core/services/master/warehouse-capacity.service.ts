...
export class WarehouseCapacityService {
  ...
  getBookingCapacityOperation(warehouseCode: string,operationType:string,bookingDate: Date): Observable<WarehouseCapacityDto[]> {    

    var addedHours = bookingDate.getTime() + (3600000*7);
    var newDate = new Date(addedHours);

    return this.apiService.get(
      `${this.apiController}/${newDate.toISOString()}/${warehouseCode}/${operationType}`
    );
  }
