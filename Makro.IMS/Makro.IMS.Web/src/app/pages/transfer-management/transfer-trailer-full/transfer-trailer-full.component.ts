...
@UntilDestroy()
@Component({
  templateUrl: './transfer-trailer-full.component.html',
})
export class TransferTrailerFullComponent implements OnInit {
  msgs: Message[] = [];
  ...
  fetchData() {
    ...
    this.trailerService
      ...
      .pipe(untilDestroyed(this))
      .subscribe((data) => {
        this.trailers = data.filter((trailer) => trailer.status === 'Full');
      });
  }
