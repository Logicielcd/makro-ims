...
import { ShuntSearchEmptyComponent } from './shunt-search-empty/shunt-search-empty.component';
import { YardSearchEmptyComponent } from './yard-search-empty/yard-search-empty.component';

@UntilDestroy()
@Component({
  selector: 'app-assign-job-empty',
  templateUrl: './assign-job-empty.component.html',
  providers: [ConfirmationService, MessageService],
})
export class AssignJobEmptyComponent implements OnInit {
  trailer: Trailer;
  shunt: Shunt;
  yard: Yard;
  ...
  openShuntSearch() {
    const ref = this.dialogService.open(ShuntSearchEmptyComponent, {
      header: 'Select Shunt',
  ...
  openYardSearch() {
    const ref = this.dialogService.open(YardSearchEmptyComponent, {
      header: 'Select Yard',
      width: window.innerWidth <= 768 ? '100vw' : '70vw',
      contentStyle: { overflow: 'auto' },
      baseZIndex: 10000,
    ...
    ref.onClose.subscribe((result: { yard: Yard } | undefined) => {
      if (result) {
  ...
  createJob() {
    ...
    const request: JobTransferRequest = {
      ...
      locationType: this.yard?.locationNo,
    };
