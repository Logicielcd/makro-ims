...
export class SlotTimeComponent implements OnInit {
  ...
  ngOnInit()
  {
    ...
    if(this.bookingDate.getDay() === 0){
      this.weekFirstDay = 6;
      this.workDays.push(6);
      this.workDays.push(this.bookingDate.getDay());
    }
    else{
      this.weekFirstDay = this.bookingDate.getDay() - 1;
  ...
  createBooking(): void
  {
    ...
    // add booking slot
    if(this.bookingSlot.internalDoorId !== undefined){
      ...
      let doorId: number;
      doorId = this.bookingSlot.internalDoorId;

      this.bookingDataSource.push({Id: 0, DoorId: +doorId, Subject: this.supplierName
        , Location: this.supplierName,Description: '-',StartTime: new Date(this.bookingSlot.startTime).toJSON(),EndTime: new Date(this.bookingSlot.endTime).toJSON()
    ...
    this.bookingHeaders.forEach(booking => {
      ...
      let fontColorCode: number = -16777216;
      let isReadOnly: boolean = false;
      if(this.user.userType == "DHL" || this.supplierCode == booking.supCode){
  ...
  public onPopupOpen(args: PopupOpenEventArgs): void
  {
    ...
    if (args.type === 'Editor') {
      ...
      if(this.bookingSlot.dockDoor !== undefined && args.data.Id === undefined){
        args.cancel = true;
        return;
      }
      else{
        if(this.bookingSlot.dockDoor !== undefined){
          isEdit = true;
        }
      }
      ...
      let startDate: Date = new Date(startElement.value);
      ...
      //args.cancel = !this.scheduleObj.isSlotAvailable(startDate, endTime);
  ...
  onPopupClose(args: PopupCloseEventArgs) : void {
    ...
    if (args.type === 'Editor' && !isNullOrUndefined(args.data)) {
        ...
        if (subjectElement ) {
            (<{ [key: string]: Object }>(args.data)).Subject = this.supplierName;
        }
        ...
        (<{ [key: string]: Object }>(args.data)).Subject = this.supplierCode;
        (<{ [key: string]: Object }>(args.data)).Location = this.supplierName;
  ...
  customEndFn(args) {
    ...
    return new Date(startElement.value) < new Date(args.value);
  }
