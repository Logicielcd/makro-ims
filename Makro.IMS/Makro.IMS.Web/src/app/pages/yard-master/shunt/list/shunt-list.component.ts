...
@UntilDestroy()
@Component({
  templateUrl: './shunt-list.component.html',
})
export class ShuntListComponent implements OnInit, OnDestroy {
  dataSource: Shunt[] = [];
  ...
  ngOnInit() {
    ...
    this.pagingService.pagedData
      .pipe(
        ...
        switchMap((sieve) => this.shuntService.getPaged(sieve)),
        untilDestroyed(this)
      )
      .subscribe({
        next: (paged) => {
