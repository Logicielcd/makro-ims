...
@UntilDestroy()
@Component({
  templateUrl: './warehouse-capacity-list.component.html',
})
export class WarehouseCapacityListComponent implements OnInit, OnDestroy {
  cols: any[];
  dataSource: WarehouseCapacity[] = [];
  rowCount: number = 0;
  msgs: Message[] = [];
  canModify = false;
  ...
  //  this.canModify = true;
    ...
    this.pagingService.pagedData
      .pipe(
        ...
        switchMap((sieve) => 
          this.warehouseCapacityService.getPaged(sieve)
        ),
        untilDestroyed(this)
      )
      .subscribe({
