...
// import { Department } from '@core/models/master/department';
...
export class AuthService {
  ...
  // private depsObserver = new BehaviorSubject<Department[]>([]);
  // availableDeps$ = this.depsObserver.asObservable();

  // private depObserver = new BehaviorSubject<Department>(null);
  // currentDep$ = this.depObserver.asObservable();
  ...
  // setDeps(deps: Department[]) {
  //   this.depsObserver.next(deps);
  // }

  // selectDep(dep: Department) {
  //   this.depObserver.next(dep);
  // }
  ...
  userRegister(userRegister: UserRegister) {
    return this.httpClient
    .post(`${environment.apiUrl}/userregister/register`, userRegister)
    .pipe(
      map((data) => {
        return true;
      }),
  ...
  forgotPassword(userRegister: UserRegister) {
    return this.httpClient
    .post(`${environment.apiUrl}/userregister/forgotpassword`, userRegister)
    .pipe(
      map((data) => {
        return true;
      }),
  ...
  validateToken(payload: TokenModel) {    
    return this.httpClient
      .post(`${environment.apiUrl}/auth/validate-token`, payload);
      // .pipe(
      //   map((data) => {
      //     var token = data as TokenModel;
      //     this.setToken(token);
